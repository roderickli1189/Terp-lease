...
function ListingIdPage() {
  ...
  const fetchListingId = async (listingId) => {
    const getListing = await fetch(
      "http://127.0.0.1:5000/listings/" + String(listingId)
    );
    ...
    <>
      ...
      <div className="card">
        <div className="card-body items-center">
          ...
          <div>
            Semester:{" "}
            {listing.semester ? listing.semester : "Semester not specified"}
          </div>
          <div>
            Roommate Gender:{" "}
            {listing.gender ? listing.gender : "Gender not specified"}
          </div>
          <div>
            Description:{" "}
            {listing.description
          ...
          <div>
            Posting Date: {new Date(listing.postDate).toLocaleDateString()}
          </div>
